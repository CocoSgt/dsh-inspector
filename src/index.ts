...
function checkCwd(cwd: string): string {
  ...
  if (!stats.isDirectory()) throw new Error(`projectFiles: cwd 不是目录: ${cwd}`)
  return resolve(cwd)
}
...
export class ProjectFilesGateway extends TypertRemoteService {
  ...
  private resolveAddress(cwd: string, scope: string, dir: string, name: string): string {
  ...
  private resolveSkillRoot(cwd: string, root: string): string {
    const resolvedCwd = checkCwd(cwd)
    const projectRoot = findProjectRoot(resolvedCwd)
    const home = dshHome()
    const agentsHome = join(homedir(), '.agents')
    ...
    if (absolute === undefined) throw new Error(`projectFiles: 不支持的技能根: ${JSON.stringify(root)}`)
    return absolute
  }

  /** 解析技能文件地址(root 相对路径,必须命中 .md 且不越根)。 */
  private resolveSkillFile(cwd: string, root: string, skillPath: string): string {
  ...
  readSkillFile(cwd: string, root: string, skillPath: string): ReadResult {
    ...
    const stats = statOf(target)
    if (stats === undefined) throw new Error(`projectFiles: 技能文件不存在: ${skillPath}`)
  ...
  writeSkillFile(cwd: string, root: string, skillPath: string, content: string): WriteResult {
    ...
    const stats = statOf(target)
    if (stats === undefined) throw new Error(`projectFiles: 写入后无法读取状态: ${skillPath}`)
